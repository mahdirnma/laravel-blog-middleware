                ...
                <form action="{{route('posts.update',compact('post'))}}" method="post" class="w-full h-full flex flex-row-reverse">
                    ...
                    <div class="w-1/2 h-full flex flex-col items-end pr-20 relative">
                        ...
                        <div class="w-5/6 h-auto flex flex-row-reverse justify-between pt-4 mb-6">
                            ...
                            <textarea name="description" id="description" cols="10" rows="10" class="w-2/5 h-32 rounded outline-0 p-2 border border-gray-400">{{$post->description}}</textarea>
                            @error('description')
                                <p class="text-red-700">{{$message}}</p>
                            @enderror
                        </div>
                    ...
                    <div class="w-1/2 h-full flex flex-col items-end pr-20">
                        <div class="w-5/6 h-auto flex flex-row-reverse justify-between pt-4 mb-6">
                            <label for="published_at" class="font-semibold ml-5">: published at</label>
                            <input type="date" name="published_at" value="{{$post->published_at}}" id="published_at" class="w-2/5 h-8 rounded outline-0 p-2 border border-gray-400">
                            @error('published_at')
                            <p class="text-red-700">{{$message}}</p>
                            @enderror
                        </div>
                        ...
                        <div class="w-5/6 h-auto flex flex-row-reverse justify-between pt-4 mb-6">
                            <label for="tag" class="font-semibold ml-5">: tag</label>
                            <select name="tag_id" id="tag" class="w-2/5 h-8 rounded outline-0 px-2 border border-gray-400">
                                @foreach($tags as $tag)
